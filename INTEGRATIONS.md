...
- `freeProductUrl`: Always free product URL. Until supplied, the “Start building” link stays hidden and the card offers only “Ask about this plan”. Supplying it reveals the product link alongside the enquiry link; it never replaces it.
- `enquiryEndpoint`: HTTPS service accepting JSON `{name,email,plan,message,company}` with CORS allowing the website origin (`company` is a honeypot field, always empty for real visitors). It must validate fields, limit requests, handle spam, and deliver/store the enquiry before returning 2xx. A non-2xx response displays an error and keeps the visitor’s entries. The current fallback prepares an email and explicitly does not claim delivery. An Azure Function implementation (Azure Communication Services Email) is ready to deploy in [`azure-function/`](azure-function/README.md) — follow its README, then set the deployed URL here.
...
- `appInsightsConnectionString`: Azure Application Insights connection string for page-view/traffic analytics via `consent.js`. Connection strings for Application Insights aren't secrets (they're meant to be public), but leave this empty until a resource exists — the banner and script loader are inert with it unset. Create an Application Insights resource in the Azure portal, copy its Connection String from the Overview pane, and paste it here. `consent.js` shows an accept/reject cookie banner on first visit (skipped entirely if the browser sends Global Privacy Control or Do Not Track) and only loads the SDK after "Accept"; the choice persists in `localStorage`. The "Cookie settings" footer link reopens the banner, and rejecting after accepting deletes the `ai_user` and `ai_session` cookies. If you add or change any cookie or tracking service, update the cookies table in the privacy policy at the same time.

No secrets belong in this public configuration. Email delivery (Azure Function) and consent-based page-view analytics (Application Insights) are connected; the custom `analyticsEndpoint` events, attribution and a reporting dashboard are not. Product-side signup/activation events require changes in the product itself.
...
- The privacy policy and terms of use cover the website and enquiries only, not the product. Have both reviewed by a solicitor. Confirm the retention periods stated in the privacy policy (24 months for enquiries, 90 days for analytics) and set Application Insights retention in Azure to match. Product privacy information and data processing terms are still needed separately.
- Fonts are self-hosted from `fonts/` (Figtree, SIL Open Font License in `fonts/OFL.txt`). Don't reintroduce Google Fonts links; the privacy policy no longer lists Google as a recipient.
...
No pilot, testimonials, savings figures, certifications or launch dates have been invented. There are two plans, framed around the mission rather than price. Always free carries the Osteoblast builder only and is promised free permanently — do not introduce limits or a trial period without changing that promise everywhere. Osteoblast is described as building agents “to DTAC expectations” rather than as DTAC-compliant, because Always free includes no assurance service and the DTAC assessment stays with the customer. Founder is in beta testing, adds the Osteoclast evaluator and Osteocite Assist documentation drafting, is joined by contacting us directly, and shows no price; keep money language (“pricing”, “purchase”) off the pages. Clinical safety officer, DCB0129 and organisation-wide support are described as in development and outside both plans, not as plans of their own.
